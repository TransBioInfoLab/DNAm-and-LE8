---
title: "Read in and NOMAS EPIC112 DNA methylation data"
author: 
...
```{r load_pheno}
pheno_df <- read.csv(file.path(pheno_dir, "Raw_Metadata_EPIC112.csv"))
```

## Get RGSet Data

```{r load_rgset}
RGSet <- readRDS(file.path(raw_dir, "RGSet_EPIC112.RDS"))
```
...
```{r save}
saveRDS(dnam_data, file = file.path(process_dir, "DNAm_Data_EPIC112.RDS"))
```
