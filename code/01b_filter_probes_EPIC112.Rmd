---
title: "Read in NOMAS EPIC112 data, get detection P scores, filter out unwanted probes, and convert to beta values"
author: 
...
```{r load_data}
dnam_data <- readRDS(file.path(data_dir, "DNAm_Data_EPIC112.RDS"))
...
```{r save}
saveRDS(dnam_data, file = file.path(data_dir, "Preprocessed_Beta_EPIC112.RDS"))
write.csv(
  detp_df,
  file = file.path(probe_dir, "detectionP_Failures_EPIC112.csv"),
  row.names = FALSE
)
write.csv(
  na_df,
  file = file.path(probe_dir, "missing_probes_EPIC112.csv"),
  row.names = FALSE
)
row_df <- dnam_data %>%
  SummarizedExperiment::rowData() %>%
...
write.csv(
  row_df,
  file = file.path(probe_dir, "Preprocssed_Probes_EPIC112.csv"),
  row.names = TRUE
)
write.csv(
  pheno_df,
  file = file.path(pheno_dir, "Preprocssed_Metadata_EPIC112.csv"),
  row.names = TRUE
)
```
