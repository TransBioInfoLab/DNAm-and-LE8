...
```{r load_probes}
dnam_data <- readRDS(file.path(data_dir, "Preprocessed_Beta_EPIC112.RDS"))
pheno_df <- dnam_data %>%
...
```{r load_detp}
detp_df <- read.csv(file.path(probe_dir, "detectionP_Failures_EPIC112.csv")) %>%
...
saveRDS(dnam_data, file.path(data_dir, "Imputed_Autosomal_Batched_EPIC112.RDS"))
```
...
## Check if Any Probes are still NAs

```{r check_na}
na_count <- rowSums(is.na(SummarizedExperiment::assays(dnam_data)$dnam))
na_count <- na_count[na_count > 0]

na_df <- data.frame(
  cpg = names(na_count),
  count = as.integer(na_count)
)
...
These are probes that have non-NAs in at most 1 sample. They can't be imputed

```{r save}
dnam_data <- dnam_data[!(row.names(dnam_data) %in% na_df$cpg),]
message(nrow(dnam_data), " probes had no NAs or coulde be imputed")
saveRDS(dnam_data, file.path(data_dir, "Imputed_Autosomal_EPIC112.RDS"))
```
